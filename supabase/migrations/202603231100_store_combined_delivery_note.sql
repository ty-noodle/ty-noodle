...
-- Replaces per-order delivery note creation with a single combined note
-- covering all order rounds for one store on one date.
--
-- Key differences from create_delivery_note:
--   - Accepts p_order_ids uuid[]  (array) instead of p_order_id uuid
--   - Creates ONE delivery note using p_order_ids[1] as the FK reference
--   - Confirms all orders in the array (submitted → confirmed)
--   - Recomputes fulfillment_status for each order via order_item_id linkage
--     (not via delivery_notes.order_id, which only points to the primary order)
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- Insert DN header (primary order as FK)
  ...
  -- Recompute fulfillment_status for EACH order via order_item linkage
  -- (cannot use dn.order_id since only the primary order is stored there)
