-- ─────────────────────────────────────────────────────────────────────────────
-- Delivery Notes
-- Tracks actual deliveries against orders.
-- fulfillment_status on orders: pending | partial | complete
-- ─────────────────────────────────────────────────────────────────────────────
...
alter table public.orders
  ...
  constraint orders_fulfillment_status_check check (
    fulfillment_status in ('pending', 'partial', 'complete')
  );

create index if not exists orders_org_fulfillment_idx
  on public.orders (organization_id, fulfillment_status, order_date desc);

-- 2. Atomic counter for DN numbers
create table if not exists public.delivery_note_counters (
  organization_id uuid primary key references public.organizations(id) on delete cascade,
  last_number     bigint not null default 0
);
revoke all on public.delivery_note_counters from anon, authenticated;

-- 3. DN number function  →  DN-YYYYMMDD-XXXXX
...
begin
  ...
  on conflict (organization_id) do update
    set last_number = delivery_note_counters.last_number + 1
  returning last_number into v_next;

  return 'DN-' || to_char(p_delivery_date, 'YYYYMMDD') || '-' || lpad(v_next::text, 5, '0');
end;
$$;

-- 4. Delivery notes header
create table if not exists public.delivery_notes (
  ...
  constraint delivery_notes_org_number_unique unique (organization_id, delivery_number),
  constraint delivery_notes_status_check check (status in ('confirmed', 'cancelled'))
);

create index if not exists delivery_notes_org_order_idx
  on public.delivery_notes (organization_id, order_id);
create index if not exists delivery_notes_org_date_idx
  on public.delivery_notes (organization_id, delivery_date desc, created_at desc);
...
create table if not exists public.delivery_note_items (
  ...
  product_sale_unit_id  uuid references public.product_sale_units(id) on delete set null,
  ...
  created_at            timestamptz not null default timezone('utc', now()),
  constraint delivery_note_items_qty_check check (
    quantity_delivered > 0 and quantity_in_base_unit > 0
  )
);

create index if not exists delivery_note_items_dn_idx
  on public.delivery_note_items (delivery_note_id);
create index if not exists delivery_note_items_order_item_idx
  on public.delivery_note_items (order_item_id);

-- 6. RLS — service role only (same pattern as orders)
...
revoke all on public.delivery_note_items from anon, authenticated;

drop policy if exists delivery_notes_deny_api_access on public.delivery_notes;
create policy delivery_notes_deny_api_access
  on public.delivery_notes as restrictive for all
  to anon, authenticated using (false) with check (false);

drop policy if exists delivery_note_items_deny_api_access on public.delivery_note_items;
create policy delivery_note_items_deny_api_access
  on public.delivery_note_items as restrictive for all
  to anon, authenticated using (false) with check (false);

-- 7. Atomic create_delivery_note function
--    - Gets next DN number
--    - Inserts DN header + items
--    - Issues inventory movements (type: 'issue')
--    - Updates product stock_quantity & reserved_quantity
--    - Recomputes and updates order.fulfillment_status
--    Returns the delivery_number text
